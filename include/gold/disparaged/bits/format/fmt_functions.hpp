...
#include <gold/bits/iterator/dummy_counting_iterator.hpp>
#include <gold/bits/iterator/enumerated_iterator.hpp>
...
namespace gold {
    ...
    namespace __format {
        ...
        constexpr Out format_each_to_(Out out, T&& item, gold::formatter<__format::decay_t_<T>>& formatter_, format_specs specs = {}) {
            ...
            const int fmt_width_ = [&]{
                gold::dummy_counting_iterator<char> iter_;
    ...
    /// formatted_size
    template <typename... Args>
        requires (formattable_to<__format::decay_t_<Args>, gold::dummy_counting_iterator<char>> && ...)
    constexpr std::size_t formatted_size(std::string_view fmt, Args&&... args) {
        gold::dummy_counting_iterator<char> iter_;
        iter_ = vformat_to<
            gold::dummy_counting_iterator<char>,
    ...
    constexpr format_to_n_result<Out> format_to_n(Out out, std::iter_difference_t<Out> n, std::string_view fmt, Args&&... args) {
        ...
        buffer_.setfill('\0');
        auto buf_ = vformat_to(gold::enumerated_iterator(buffer_.data()), fmt, make_format_args(std::forward<Args>(args)...));
