...
namespace gold {
    ...
    namespace __format {
        ...
        /// __format::try_do_vformat_to_2
        template <typename Sink, typename Out>
        [[gnu::always_inline]]
        constexpr __format::result<Out> try_do_vformat_to_2(Out& out, std::string_view fmt, const gold::format_args& args) {
        ...
        constexpr __format::result<Out> try_do_vformat_to(Out out, std::string_view fmt, const gold::format_args& args) {
            if consteval {
                return __format::try_do_vformat_to_2<__format::choose_iter_sink_t<Out, true>>(out, fmt, args);
            } else {
                return __format::try_do_vformat_to_2<__format::choose_iter_sink_t<Out, false>>(out, fmt, args);
            }
        }

        /// __format::do_vformat_to
        template <typename Out>
        constexpr Out do_vformat_to(Out out, std::string_view fmt, const gold::format_args& args) {
    ...
    [[gnu::always_inline]] inline constexpr Out vformat_to(Out out, std::string_view fmt, gold::format_args args) {
        return __format::do_vformat_to(std::move(out), fmt, args);
    }

    /// vformat
    [[nodiscard]] inline constexpr std::string vformat(std::string_view fmt, gold::format_args args) {
