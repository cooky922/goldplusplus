...
namespace gold::__format {
    ...
    struct scanner {
        ...
        constexpr virtual __format::result<void> format_arg(std::size_t) noexcept {
            return {};
        }
    ...
    struct formatting_scanner : scanner {
        ...
        constexpr explicit formatting_scanner(format_context& fc, std::string_view s)
        : scanner(s), m_fc_(fc) {}
        ...
        constexpr __format::result<void> format_arg(std::size_t id) noexcept override {
            using handle  = gold::format_arg::handle;
            return gold::visit_format_arg([this]<typename T>(T& arg) noexcept -> __format::result<void> {
                using Type      = T;
            ...
            if constexpr (sizeof...(Args) != 0) {
                gold::__algo::visit_with_index<sizeof...(Args)>(id, [&]<std::size_t I> {
                    if (ec == __fmt_error_code::none) {
                        using Formatter = gold::formatter<__format::normalize_type<Args...[I]>>;
                        Formatter f;
