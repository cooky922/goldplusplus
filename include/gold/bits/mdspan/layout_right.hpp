// <gold/bits/mdspan/layout_right.hpp> - gold++ library
...
#pragma once
#ifndef __GOLD_BITS_MDSPAN_LAYOUT_RIGHT_HPP
#define __GOLD_BITS_MDSPAN_LAYOUT_RIGHT_HPP
    ...
    class layout_right::mapping {
      ...
      public:
        ...
        constexpr std::array<index_type, extents_type::rank()> strides() const noexcept {
            ...
            index_type s = 1;
            for (rank_type i = extents_type::rank() - 1; i > 0; --i) {
                s *= extents().extent(i);
                result[i] = s;
            }
            return result;
        ...
        constexpr index_type operator()(Indices... is) const noexcept {
            ...
            return gold::pack_apply_index<sizeof...(Indices)>([&]<std::size_t... Pos> {
                index_type res = 0;
                ((res = static_cast<index_type>(is) + m_extents_.extent(Pos) * res), ...);
                return res;
            });
        }
        ...
        constexpr index_type stride(rank_type r) const noexcept requires (extents_type::rank() > 0) {
            return extents().mf_rev_prod_of_extents_(r);
        }
...
#endif // __GOLD_BITS_MDSPAN_LAYOUT_RIGHT_HPP
