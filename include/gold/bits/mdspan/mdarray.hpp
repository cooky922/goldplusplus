...
#include <gold/bits/__util/iota_array.hpp>
...
namespace gold {
    ...
    namespace __mdspan {
        ...
        inline constexpr std::size_t static_extents_size<std::extents<IndexType, Extents...>> = (Extents * ... * 1);
    ...
    class mdarray {
      ...
      private:
        ...
        static constexpr auto sf_val_to_array_(const value_type& val) requires s_is_container_array_ {
            constexpr auto [... Is] = __util::iota_array<__mdspan::static_extents_size<extents_type>>;
      ...
      public:
        ...
        mdarray(const std::mdspan<U, OtherExtents, OtherLayoutPolicy, Accessor>& other)
        ...
        constexpr explicit(!std::is_convertible_v<const typename OtherLayoutPolicy::template mapping<OtherExtents>&, mapping_type> ||
                           !std::is_convertible_v<typename Accessor::reference, value_type>)
        mdarray(const std::mdspan<U, OtherExtents, OtherLayoutPolicy, Accessor>& other, const Alloc& alloc)
        ...
        constexpr decltype(auto) operator[](this auto& self, std::span<OtherIndexType, rank()> indices) {
            constexpr auto [...Is] = __util::iota_array<rank()>;
        ...
        constexpr operator std::mdspan<U, OtherExtents, OtherLayoutType, OtherAccessorType>() {
            return std::mdspan(container_data(), m_map_);
        }

        template <typename OtherAccessorType = std::default_accessor<element_type>>
        constexpr std::mdspan<element_type, extents_type, layout_type, OtherAccessorType>
        to_mdspan(const OtherAccessorType& a = {}) {
            return std::mdspan<element_type, extents_type, layout_type, OtherAccessorType>(
                container_data(), m_map_, a
            );
        }

        template <typename OtherAccessorType = std::default_accessor<const element_type>>
        constexpr std::mdspan<const element_type, extents_type, layout_type, OtherAccessorType>
        to_mdspan(const OtherAccessorType& a = {}) const {
            return std::mdspan<const element_type, extents_type, layout_type, OtherAccessorType>(
                container_data(), m_map_, a
            );
        }
    ...
    mdarray(const std::extents<IndexType, Extents...>&, const Container&)
    -> mdarray<typename Container::value_type, std::extents<IndexType, Extents...>, std::layout_right, Container>;
    ...
    template <typename IndexType, std::size_t... Extents, typename Container>
    mdarray(const std::extents<IndexType, Extents...>&, Container&&)
    -> mdarray<typename Container::value_type, std::extents<IndexType, Extents...>, std::layout_right, Container>;
    ...
    mdarray(const std::mdspan<T, Extents, Layout, Accessor>&)
    -> mdarray<std::remove_cv_t<T>, Extents, Layout>;

    template <typename IndexType, std::size_t... Extents, typename Container, typename Alloc>
    mdarray(const std::extents<IndexType, Extents...>&, const Container&, const Alloc&)
    -> mdarray<typename Container::value_type, std::extents<IndexType, Extents...>, std::layout_right, Container>;
    ...
    template <typename IndexType, std::size_t... Extents, typename Container, typename Alloc>
    mdarray(const std::extents<IndexType, Extents...>&, Container&&, const Alloc&)
    -> mdarray<typename Container::value_type, std::extents<IndexType, Extents...>, std::layout_right, Container>;
    ...
    mdarray(const std::mdspan<T, Extents, Layout, Accessor>&, const Alloc&)
    -> mdarray<std::remove_cv_t<T>, Extents, Layout>;
