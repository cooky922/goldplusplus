...
namespace gold::tuples {
    ...
    namespace __tuples {
        ...
        template <typename T>
            requires requires { { gold::constant_arg_t<T::extent>::value } -> std::convertible_to<std::size_t>; }
                  || requires { { gold::constant_arg_t<T::size()>::value } -> std::convertible_to<std::size_t>; }
        struct array_like_extent<T> {
            inline static constexpr std::size_t value = [] -> std::size_t {
                if constexpr (requires { typename gold::constant_arg_t<T::extent>; })
                    return T::extent;
                else
                    return T::size();
            }();
