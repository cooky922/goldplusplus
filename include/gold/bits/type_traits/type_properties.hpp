...
namespace gold {
    ...
    /// is_structural_type_v
    template <typename T>
    inline constexpr bool is_structural_type_v = requires { []<T N>(constant_arg_t<N>){}; };

    /// is_structural_type
    template <typename T>
    struct is_structural_type : std::bool_constant<is_structural_type_v<T>> {};
