// <gold/bits/functional/function_ref.hpp> - gold++ library
...
#pragma once
#ifndef __GOLD_BITS_FUNCTIONAL_FUNCTION_REF_HPP
#define __GOLD_BITS_FUNCTIONAL_FUNCTION_REF_HPP
...
namespace gold {
    ...
    namespace __functional {
        ...
        /// __functional::fn_ref_kind
        enum class fn_ref_kind : unsigned char {
            stateful,
            stateless,
            none
        };
    ...
    namespace __functional {
        ...
        /// __functional::fn_ref_base
        template <typename, bool, bool>
        class fn_ref_base;

        template <typename R, typename... Args, bool Const, bool Noexcept>
        class fn_ref_base<R(Args...), Const, Noexcept> {
          private:
            ...
            __functional::fn_ref_kind m_kind_;

            constexpr bool mf_is_stateful_() const noexcept {
                return m_kind_ == __functional::fn_ref_kind::stateful;
            }

            constexpr bool mf_is_stateless_() const noexcept {
                return m_kind_ == __functional::fn_ref_kind::stateless;
            }

            constexpr bool mf_is_none_() const noexcept {
                return m_kind_ == __functional::fn_ref_kind::none;
            }
          ...
          public:
            /// default ctor
            constexpr fn_ref_base() noexcept
            : m_state_(nullptr), m_kind_(__functional::fn_ref_kind::none) {}

            /// copy ctor
            constexpr fn_ref_base(const fn_ref_base&) noexcept = default;
            ...
            constexpr fn_ref_base(F&& f) noexcept
            : m_state_(nullptr), m_stateless_fn_(gold::voidify(f)),
              m_kind_(__functional::fn_ref_kind::stateless) {}
            ...
            : m_state_(nullptr), m_stateless_fn_(f),
              m_kind_(__functional::fn_ref_kind::stateless) {
                if (f == nullptr)
                    m_kind_ = __functional::fn_ref_kind::none;
              }
            ...
            constexpr fn_ref_base(constant_arg_t<M>) noexcept
            ...
            constexpr fn_ref_base(constant_arg_t<M>, T& state) noexcept
            ...
            }), m_kind_(__functional::fn_ref_kind::stateful) {}

            /// ctor: NTTP member pointer with state pointer
            template <auto M, typename T>
                requires std::is_member_pointer_v<decltype(M)>
                      && s_is_invocable_using_<decltype(M), maybe_const_t<T>*>
            constexpr fn_ref_base(constant_arg_t<M>, maybe_const_t<T>* state) noexcept
            : m_state_(state),
              m_stateful_fn_([](void* state, Args... args) noexcept(Noexcept) -> R {
                return gold::invoke_r<R>(M,
            ...
            constexpr fn_ref_base(constant_arg_t<M>) noexcept
            : m_state_(nullptr), m_stateless_fn_(M), m_kind_(__functional::fn_ref_kind::stateless) {}

            /// ctor: nullptr
            constexpr fn_ref_base(std::nullptr_t) noexcept
            : m_state_(nullptr), m_kind_(__functional::fn_ref_kind::none) {}
            ...
            /// copy assignment
            constexpr fn_ref_base& operator=(const fn_ref_base&) noexcept = default;

            /// assignment
            template <typename F>
            constexpr fn_ref_base& operator=(F&& f) noexcept
                requires requires { fn_ref_base(std::forward<F>(f)); }
            {
                fn_ref_base(std::forward<F>(f)).do_swap(*this);
                return *this;
            }
            ...
            /// do_swap
            constexpr void do_swap(fn_ref_base& other) noexcept {
            ...
            /// do_eq
            constexpr bool do_eq(const fn_ref_base& other) const noexcept {
            ...
            constexpr R operator()(Args... args) const noexcept(Noexcept) {
                ...
                switch (m_kind_) {
                    case __functional::fn_ref_kind::stateless:
                        return m_stateless_fn_(std::forward<Args>(args)...);
                    case __functional::fn_ref_kind::stateful:
    ...
    /// deduction guides for gold::function_ref
    template <typename F>
    ...
    function_ref(constant_arg_t<F>) -> function_ref<std::remove_pointer_t<decltype(F)>>;

    template <auto F, typename T = decltype(F)>
        requires std::is_member_pointer_v<T>
    function_ref(constant_arg_t<F>, auto&) -> function_ref<
        gold::conditional_t<
...
#endif // __GOLD_BITS_FUNCTIONAL_FUNCTION_REF_HPP
